...
*
* Contributors:
*  Endre Szalai - initial implementation and initial documentation
*  Endre Kulcsar
*  Gabor Bettesch
*  Gabor Szalai
*  Laszlo Tamas Zeke
...
Decode_GRE2_header(TTCN_Buffer & bb, IP__GRE2__header & field)
{
  ...
  else if (field.protocol__type() == c__ip__gre__proto__transparentEthernetBridging)
    return c__ip__gre__proto__transparentEthernetBridging;
  else
  {
    unsigned int tmp = field.protocol__type();
    TTCN_error("Unknown protocol-type in GRE2 header "
               "(must be 0x0800 for IPv4 or 0x86DD for IPv6 or 0x6558 for Transparent Ethernet Bridging): 0x%02x", tmp);
  }
}
